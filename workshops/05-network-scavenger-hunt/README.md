...
**Example files in this folder:** `commands.sh` (reference commands for nmap, arp, ss)
...
- **nmap:** Port scan. See `commands.sh` in this folder for examples: host discovery, fast port scan, service detection.
...
1. **Know your subnet:** Facilitator gives the range (e.g. 192.168.10.0/24).
2. **Host discovery:** `nmap -sn 192.168.10.0/24` or ping sweep + `arp -a`.
3. **List:** Produce a list of responding IPs. Exclude team's own Pi and default gateway if desired.
...
1. **Port scan each host:** For each IP from Phase 1, run e.g. `nmap -F <ip>` or `nmap -p 22,80,443,8080,9090 <ip>`. Goal: find at least one open port per device.
2. **Record:** Build a table: IP | open port(s) | (optional) guessed service.
3. **Service guess (optional):** `nmap -sV -p <port> <ip>` or guess from port number.

**Handout:** "Phase 2: For each discovered device, find at least one open port. Optional: guess the service. Submit the full map."
...
2. **Answer key:** Facilitator has the canonical list (all expected IPs and at least one open port per device).
3. **Scoring:** Full marks = all devices present with at least one correct port. First correct (or highest score) wins. **Prize:** Raspberry Pi for the winning team.
...
- **No hosts found:** Confirm the team's Pi is on the same subnet (`ip addr`); confirm targets are powered and connected; try pinging the gateway first.
- **Wrong port reported:** Answer key should list what you actually configured. If a team reports a port that's open but not on the key, accept it if verifiable.

---

## If you have extra time

- Add "mystery" devices: e.g. a second router, a NAS, or a locked-down Pi with only one unusual port open. Name them in the key.
