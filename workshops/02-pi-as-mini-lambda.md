...
1. **Create a handler script** (e.g. `/home/pi/handler.py`):
   - When run, it writes the current time to a file (e.g. `/home/pi/last_run.txt`) and prints “OK”.
   - Example:
     ```python
     #!/usr/bin/env python3
     ...
     print("OK")
     ```
2. **Trigger by HTTP:** Use Flask (or another microframework) to run this logic when a route is hit:
   - `GET /run` → run the handler logic (update file, return `{"status": "ok"}`).
   - Run Flask on port 5000: `flask run --host=0.0.0.0 --port=5000`.
3. **Trigger by cron:** Add a crontab entry to run the script every minute: `* * * * * /usr/bin/python3 /home/pi/handler.py >> /home/pi/cron.log 2>&1`.
4. **Verify:** Call `curl http://<pi-ip>:5000/run` and check `last_run.txt`; wait a minute and check cron updated it too.
...
1. **Choose an “input” folder:** e.g. `/home/pi/inbox`.
2. **Watch the folder:** Use `pyinotify` or `watchdog` to run the handler when a new file is created. Example with `watchdog`:
   - Install: `pip install watchdog`.
   - Script that watches `inbox/` and when a file is created, runs the same “handler” (e.g. append the filename to a log and move file to `processed/`).
3. **Test:** Drop a file into `inbox/` (e.g. `echo hello > inbox/test.txt`); confirm handler ran (log updated, file moved).
...
**Goal:** Deploy a Lambda that does the same “job” (e.g. write timestamp or process a simple payload) and invoke it via HTTP.

1. **Template:** Provide a minimal Lambda (Python/Node): on invoke, it returns `{"status": "ok", "timestamp": "<utc-now>"}` or similar. Use API Gateway HTTP API or REST API to trigger it.
2. **Deploy:** Either SAM (`sam init`, add handler, `sam deploy`), Serverless Framework, or manual zip upload + API Gateway in console. Document one path (e.g. “create Lambda from zip, add API Gateway HTTP API, link to Lambda”).
3. **Invoke:** `curl https://<api-id>.execute-api.<region>.amazonaws.com/run` (or the URL you get). Compare with Pi: same “run on request” idea, no Pi to maintain.
...
(Adjust for API Gateway proxy integration if needed, e.g. return `{"statusCode": 200, "body": json.dumps({...})}`.)
...
- [ ] **Lambda template:** Zip with handler above or SAM/serverless template; instructions for creating API Gateway and linking to Lambda.
