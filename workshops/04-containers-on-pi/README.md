...
**Example files in this folder:** `Dockerfile`, `app.py`
...
**Goal:** Build and run a container that, when started, serves HTTP and returns a simple response.

1. **Use the files in this folder:** `Dockerfile` and `app.py`. Copy to the Pi. Build: `docker build -t myhandler .`. Run: `docker run --rm -p 5000:5000 myhandler`. From another machine: `curl http://<pi-ip>:5000/`.
2. **Emphasize:** This container is "the handler." We'll trigger it in different ways next.
...
1. **Option B — run on request:** Small Flask app on the Pi that on GET /run runs `docker run --rm myhandler` and captures stdout; return that as the HTTP response. Container starts, serves one "logical" request, exits.
2. Example: `result = subprocess.run(["docker", "run", "--rm", "myhandler"], capture_output=True, text=True)` then return `result.stdout` with status 200.
...
1. **File trigger:** Folder `inbox/`. When a file appears (inotify or poll), run `docker run --rm -v /home/pi/inbox:/inbox myhandler`. Script moves or deletes the file after run to avoid re-triggering.
2. **Cron trigger:** `* * * * * docker run --rm myhandler >> /home/pi/cron.log 2>&1`. Every minute the container runs once.
...
1. **Second image:** Build another Dockerfile (e.g. returns `{"message": "goodbye"}`). Tag as `myhandler2`.
2. **Change the trigger script:** Point "run on request" (and optionally cron) to `myhandler2` instead of `myhandler`.

**Prize:** "Smallest working image" or "most creative trigger" (e.g. MQTT, file drop).
...
- **Container exits immediately:** If using Flask, ensure it's bound to 0.0.0.0; for "run on request" you might use a container that prints one line and exits so the Pi script can capture stdout quickly.
